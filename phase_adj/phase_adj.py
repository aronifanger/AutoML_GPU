...
for pivot in pivots[:n]:
#for pivot in range(1,size+1):
    xopt = least_squares(ACMEentropy, [ 0.0, 0.0 ], bounds=(-180, 180), args=(FFT, pivot))
    x = xopt.x
    h = ACMEentropy(x,FFT,pivot)[0]
    results.append((h,pivot,x))
x = min(results)
cpu_time = (timer()-start) / n * size # estimated time to run all pivots
#cpu_time = timer() - start
cpu_h1 = x[0]

# dataset(size), cpu_h1, gpu_h1, cpu_time, gpu_time, speedup")
print("%s(%d)\t%f\t%f\t%f\t%f\t%f" % (dataset, size, cpu_h1, gpu_h1, cpu_time, gpu_time, cpu_time/gpu_time))
